...
public class TriggerTableHelper {
	...
	public long createTrigger(Trigger trigger, DatabaseHelper db){
		...
		values.put(KEY_TRIGGER_DEF_ID, trigger.getTriggerNumber());
		values.put(KEY_TRIGGER_TRIGGERED, trigger.getTriggered());
...
//		Log.e(LOG, selectQuery);
		...
		if(c != null){
			...
			trigger.loadTrigger(c.getInt(c.getColumnIndex(KEY_ID)), c.getInt(c.getColumnIndex(KEY_TRIGGER_DEF_ID)), c.getString(c.getColumnIndex(KEY_PARAM_1)), c.getString(c.getColumnIndex(KEY_PARAM_2)), c.getString(c.getColumnIndex(KEY_PARAM_3)), c.getInt(c.getColumnIndex(KEY_MOMENT_ID)));
			return trigger;
		}
		return null;
	...
	public void setTriggered(int triggerDefId, int triggered, DatabaseHelper db){
		SQLiteDatabase database = db.getReadableDatabase();
		
		ContentValues values = new ContentValues();
		values.put(KEY_TRIGGER_TRIGGERED, triggered);
		
		database.update(TABLE_TRIGGERS, values, KEY_TRIGGER_DEF_ID + " = ?", new String[] {String.valueOf(triggerDefId)});
	}
